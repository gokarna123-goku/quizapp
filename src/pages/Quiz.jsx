...
const Quiz = ({ questions }) => {
    ...
    const correctAnswers = questions
        .map((q, i) => (q.correct_answer === selectedAnswers[i] ? i + 1 : null))
        .filter((q) => q !== null);

    // Calculate correct and incorrect answers
    const incorrectAnswers = questions
        .map((q, i) => (q.correct_answer !== selectedAnswers[i] && selectedAnswers[i] ? i + 1 : null))
        .filter((q) => q !== null);
    ...
    return (
        <div className="w-[80%] -space-y-4 px-36 flex items-center justify-center flex-col z-20 relative">
            <div div className="w-full p-6 space-y-4 bg-neutral-600/20 border border-neutral-100/20 backdrop-blur rounded-xl" >
                < QuizTimer
